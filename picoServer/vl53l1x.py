# VL53L1X driver for MicroPython
# Kevin McAleer
# July 2022
...
0x20, # 0x46 : interrupt configuration 0->level low detection, 1-> level high, 2-> Out of window, 3->In window, 0x20-> New sample ready
...
class VL53L1X:
    ...
    def __writeReg16Bit(self, reg, value):
    ...
    def __readReg16Bit(self, reg):
        data = self.i2c.readfrom_mem(self.address, reg, 2, addrsize=16)
        return (data[0]<<8) + data[1]

    @property
    def model_id(self):
        return self.__readReg16Bit(0x010F)

    def reset(self):
        self.__writeReg(0x0000, 0x00)
        machine.lightsleep(100)
        self.__writeReg(0x0000, 0x01)
